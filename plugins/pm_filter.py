...
async def give_filter(client, message):
    ...
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        if total_results == 0:
            ...
            user_mention = message.from_user.mention if message.from_user else "User"
            return await message.reply_text(f"<b>Hᴇʏ {user_mention}, {str(total_results)} ʀᴇsᴜʟᴛs ᴀʀᴇ ғᴏᴜɴᴅ ɪɴ ᴍʏ ᴅᴀᴛᴀʙᴀsᴇ ғᴏʀ ʏᴏᴜʀ ᴏ̨ᴜᴇʀʏ {search}. \n\nTʜɪs ɪs ᴀ sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ sᴏ ᴛʜᴀᴛ ʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ғɪʟᴇs ғʀᴏᴍ ʜᴇʀᴇ...\n\nJᴏɪɴ ᴀɴᴅ Sᴇᴀʀᴄʜ Hᴇʀᴇ - {GRP_LNK}</b>")
    ...
    if query.data == "close_data":
        ...
        if not await db.has_premium_access(query.from_user.id):
            btn = [
                ...
                [InlineKeyboardButton("📱 টেলিগ্রাম ফাইল নিন", callback_data="close_data")],
                [InlineKeyboardButton("❌ বন্ধ করুন", callback_data="close_data")]
            ]
            
            daily_count = await db.get_daily_stream_count(query.from_user.id)
            text = f"""<b>⚠️ আপনার দৈনিক ডাউনলোড লিমিট শেষ!</b>

<b>📊 আজকের ব্যবহার:</b> {daily_count}/2 ডাউনলোড সম্পন্ন

<b>🚫 ফ্রি ইউজার সীমাবদ্ধতা:</b>
- দৈনিক মাত্র ২টি ডাউনলোড
- সীমিত স্পিড
- বিজ্ঞাপন দেখতে হয়

<b>🔥 আনলিমিটেড ডাউনলোড উপভোগ করতে প্রিমিয়াম নিন!</b>
...
<b>👮 প্রিমিয়াম নিতে অ্যাডমিনের সাথে যোগাযোগ করুন:</b>
📱 @neil_0998
    ...
    if query.data.startswith("file"):
        ...
        if await db.has_premium_access(query.from_user.id):
            ...
            btn = [[
                ...
                InlineKeyboardButton("🔄 Try Again", callback_data=f"del#{file_id}")
            ], [
     ...
     #   if not search:
      ...
      #      return
        ...
        if not await db.has_premium_access(user_id):
            # Check daily stream limit for free users (only for stream links, not telegram files)
            daily_count = await db.get_daily_stream_count(user_id)
            if daily_count >= 2:
                text = f"""<b>📊 আজকের স্ট্রিম লিমিট শেষ!</b>

<b>আজকের ব্যবহার:</b> {daily_count}/2 স্ট্রিম লিংক ব্যবহৃত

<b>💡 ফ্রি ইউজার লিমিটেশন:</b>
- প্রতিদিন শুধুমাত্র ২টি স্ট্রিম লিংক
- টেলিগ্রাম ফাইল সার্চ সীমাহীন ✅

<b>🔥 প্রিমিয়ামে আপগ্রেড করুন:</b>
- আনলিমিটেড স্ট্রিমিং
- দ্রুত ডাউনলোড স্পিড  
- প্রাইওরিটি সাপোর্ট
- HD কোয়ালিটি স্ট্রিমিং
...
<b>👮 প্রিমিয়াম নিতে:</b>
প্ল্যান দেখতে /plan পাঠান অথবা অ্যাডমিনের সাথে যোগাযোগ করুন

<b>⏰ আপনার লিমিট আগামীকাল রিসেট হবে!</b>
<b>📱 এখনই টেলিগ্রাম ফাইল সার্চ করতে পারবেন!</b>"""
                await query.answer(text, show_alert=True)
                return
            
            # Increment stream count for free users (only for stream link generation)
