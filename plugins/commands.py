...
async def start(client, message):
    ...
    # Handle permanent links with file_id (perm_file_id format)
    if pre == "perm":
        # This is a permanent link using file_id that won't expire
        from file_id_links import generate_file_id_links

        # Generate permanent links using file_id
        try:
            # Create file object with the file_id
            file_obj = {
                "document": {
                    "file_id": file_id,
                    "file_name": "video.mp4"
            ...
            links = generate_file_id_links(file_obj)
            ...
            if links:
                ...
                # Send message with permanent links
                await message.reply_text(
                    text="<b>Here are your permanent links that won't expire:</b>",
                    reply_markup=InlineKeyboardMarkup(button)
                )

                # Also send the actual file
                try:
                    msg = await client.send_cached_media(
                        chat_id=message.from_user.id,
                        file_id=file_id,
                        caption="<b>This file is available with permanent links that won't expire even if this message is deleted.</b>",
                        reply_markup=InlineKeyboardMarkup(button)
                    )
                    return
                except Exception as e:
                    logger.error(f"Error sending file with permanent links: {e}")
                    # Continue with normal flow if sending file fails
                    pass

        except Exception as e:
            logger.error(f"Error generating permanent links: {e}")
            # Continue with normal flow if permanent links generation fails
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            await asyncio.sleep(1)
        await sts.delete()
        k = await client.send_message(chat_id = message.from_user.id, text=f"<blockquote><b><u>🚨🚨🚨 গুরুত্বপূর্ণ নোটিশ 🚨🚨🚨</u></b>\n\n📢 <b>এই মেসেজটি <u>১০ মিনিটে</u> মুছে যাবে</b> 🕐\n<i>(কপিরাইট সমস্যার কারণে)</i>\n\n💾 <b>অনুগ্রহ করে এই মেসেজটি আপনার সেভড মেসেজে বা কোনো প্রাইভেট চ্যাটে ফরওয়ার্ড করুন</b>\n\n⚡ <i>দ্রুত ডাউনলোড করে নিন!</i></blockquote>")
        ...
        for file in files:
            ...
            if not await db.has_premium_access(message.from_user.id):
                if not await check_verification(client, message.from_user.id) and VERIFY == True:
                    btn = [[
                        InlineKeyboardButton("ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
                    ],[
                        InlineKeyboardButton("ʜᴏᴡ ᴛᴏ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ", url=VERIFY_TUTORIAL)
                    ]]
                    text = "<b>ʜᴇʏ {} 👋,\n\nʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴠᴇʀɪꜰɪᴇᴅ ᴛᴏᴅᴀʏ, ᴘʟᴇᴀꜱᴇ ᴄʟɪᴄᴋ ᴏɴ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ & ɢᴇᴛ ᴜɴʟɪᴍɪᴛᴇᴅ ᴀᴄᴄᴇꜱꜱ ғᴏʀ ᴛᴏᴅᴀʏ</b>"
                    if PREMIUM_AND_REFERAL_MODE == True:
                        text += "<b>ɪғ ʏᴏᴜ ᴡᴀɴᴛ ᴅɪʀᴇᴄᴛ ғɪʟᴇꜱ ᴡɪᴛʜᴏᴜᴛ ᴀɴʏ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴꜱ ᴛʜᴇɴ ʙᴜʏ ʙᴏᴛ ꜱᴜʙꜱᴄʀɪᴘᴛɪᴏɴ ☺️\n\n💶 ꜱᴇɴᴅ /plan ᴛᴏ ʙᴜʏ ꜱᴜʙꜱᴄʀɪᴘᴛɪᴏɴ</b>"
                    await message.reply_text(
    ...
    if not files_:
        ...
        try:
            ...
            k = await msg.reply(text=f"<blockquote><b><u>🚨🚨🚨 গুরুত্বপূর্ণ নোটিশ 🚨🚨🚨</u></b>\n\n📢 <b>এই মেসেজটি <u>১০ মিনিটে</u> মুছে যাবে</b> 🕐\n<i>(কপিরাইট সমস্যার কারণে)</i>\n\n💾 <b>অনুগ্রহ করে এই মেসেজটি আপনার সেভড মেসেজে বা কোনো প্রাইভেট চ্যাটে ফরওয়ার্ড করুন</b>\n\n⚡ <i>দ্রুত ডাউনলোড করে নিন!</i></blockquote>")
            await asyncio.sleep(600)
            await msg.delete()
...
async def plans_cmd_handler(client, message):
    ...
    await message.reply_photo(
        photo=PAYMENT_QR,
        caption=PAYMENT_TEXT.format(bkash=BKASH_NUMBER),
        reply_markup=reply_markup
    )
